...
try {
  ...
  $serverName = "10.1.9.113"; // Server name or IP address
...
$serverName = "10.1.9.113"; // O el nombre de tu servidor SQL
...
// Configuración de la conexión a SQL Server
$serverName = "10.1.9.113"; // Nombre del servidor
$connectionOptions = array(
    "Database" => "junta", // Nombre de la base de datos
    "Uid" => "SA", // Usuario
...
$queryData = "
    ...
    INNER JOIN _junta_movimientos j_mov ON j_mov.legdoc = ? AND j_doc.legajo = j_mov.legdoc
    INNER JOIN _junta_modalidades j_mod ON j_mov.codmod = j_mod.codmod AND j_mov.codmod = ? AND j_mov.tipo = ?
...
if (sqlsrv_has_rows($resultData)) {
    ...
    if (isset($row['anodoc'])) {
        ...
        echo "<th>Curso: </th><th><input type='text' name='anodoc' value='' size='11'></th>"; // Otra acción si no está definido
    }
...
<?php
if ($row['tipo'] == "titulares" && isset($obs) && isset($horas)) {
...
echo "</script>";
                ...
                echo "<th style='width:50px;'>Localidad:";
                echo "<select name='codloc' style='width: 200px;'>";
                echo "<option></option>";
                echo "<option value='USH'"; 
                if (trim($row['codloc']) == "USH") { echo " selected"; } 
                echo ">Ushuaia</option>";
                echo "<option value='RGD'"; 
                if (trim($row['codloc']) == "RGD") { echo " selected"; } 
                echo ">Rio Grande</option>";
                echo "<option value='TOL'"; 
                if (trim($row['codloc']) == "TOL") { echo " selected"; } 
                echo ">Tolhuin</option>";
                echo "<option value='ANT1'"; 
                if (trim($row['codloc']) == "ANT1") { echo " selected"; } 
                echo ">Antartida</option>";
                echo "</select></th>";
                                              echo"<th>";
...
echo "</script>";
      ...
      // Llamar a la función para mostrar campos adicionales y tablas al cargar la página
                ...
                <!--Si no se encontraron motivos de exclusión, mostrar un mensaje-->
                echo "No se encontraron motivos de exclusión en la base de datos.";
                }

                echo "</table>"; // Cierre de la tabla
                } else {
                    echo "<p>No se encontraron registros para el apellido y nombre. o no posse establecimiento </p>";
                }
           ...
           echo"<br>";
            echo" <br>";
              ...
              echo "<button type='button' class='btn btn-success' onclick='actualizarDatos()'>Actualizar Datos</button>";
              echo "</form>";
              echo "<center>";
