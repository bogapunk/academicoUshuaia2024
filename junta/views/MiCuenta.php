...
if(isset($_POST['signupSubmit'])){
    ...
    if(!empty($_POST['nombres']) && !empty($_POST['apellidos']) && !empty($_POST['email']) && !empty($_POST['telefono']) && !empty($_POST['rol']) && !empty($_POST['password']) && !empty($_POST['confirm_password'])){
		//password and confirma password comparacion
        if($_POST['password'] !== $_POST['confirm_password']){
            ...
            if($prevUser > 0){
                ...
                if($insert){
                    ...
                    $sessData['estado']['msg'] = 'Se registraste exitosamente, inicia sesión con tus credenciales.';
                }else{
                    $sessData['estado']['type'] = 'success';
                    $sessData['estado']['msg'] = 'Se registraste exitosamente, inicia sesión con tus credenciales.';// esto lo dejo asi porque en sqlserver me lo esta grabandp bien 
                }
            }
        }
    }else{
	...
	//store reset password estado into the session
    $_SESSION['sessData'] = $sessData;
    $redirectURL = ($sessData['estado']['type'] == 'success')?'panel.php':'ReiniciarPassword.php?fp_code='.$fp_code;
