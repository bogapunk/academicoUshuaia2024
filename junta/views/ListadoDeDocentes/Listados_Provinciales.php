...
try {
    // Obtener todas las modalidades y ciudades permitidas
    $sql_listado_generales = "SELECT modalidades, ciudad,id  FROM [Junta].[dbo].[_junta_listadosgenerales] WHERE ciudad LIKE :ciudad";
    $stmt = $conn->prepare($sql_listado_generales);
    
    // Asegurar que ciudad tenga el formato correcto para LIKE
    $ciudad_param = "%" . trim($ciudad) . "%";
    $stmt->bindParam(':ciudad', $ciudad_param, PDO::PARAM_STR);
    ...
    if (count($results) > 0) {
        ...
        $modalidades = implode("','", explode("-", trim($row['modalidades'], "-")));

        // Convertir ciudades a lista de valores
    ...
    WHERE j_mov.codmod IN ('$modalidades') 
      AND j_mov.codloc IN ($ciudad_str) 
    AND j_mov.anodoc = $anio 
    AND j_mov.tipo = 'titulares'
    ORDER BY j_mov.legdoc;
    ";
 var_dump($query);
 exit;
  ...
  die("Error en la consulta SQL: " . $e->getMessage());
    ...
    if ($_GET["localidad"] === "USH") {
        ...
        $pdf->Cell(35, 3, 'LIST. DIFINITIVO', 0, 1, "C");
        $pdf->SetFont('Arial', 'I', 12);
